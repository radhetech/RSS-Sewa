...
dateSelChange(selDate:any){
  ...
  this._apiService.getData('http://localhost:4000/shakhaVrut').subscribe((res:any)=>{
    res = res[1];
    this.formId = res?.id;
  if(res!=undefined){
    ...
    this.formGroup.patchValue({
      ...
      weeklyForm:{
        ...
        sanskar1Group:{
          ...
          amrutvachanSubhashit: res?.weeklyForm?.sanskar1Group?.amrutvachanSubhashit,
        },
        sanskar2Group:{
...
submitForm(form:any){
  ...
  if(this.formType =='Submit'){
    this._apiService.postData('http://localhost:4000/shakhaVrut',form.value).subscribe((res)=>{
      console.log(res)
   })
  }else{
    this._apiService.udpateRecord(`http://localhost:4000/shakhaVrut/${this.formId}`,form.value).subscribe((res)=>{
      console.log(res)
   })
  }
